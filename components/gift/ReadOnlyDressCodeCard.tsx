...
import { DressCodeIconSvg } from "@/components/ui/Icon";
...
function DressCodeHeroSVG({ color }: { color: string }) {
  return (
    <svg width="48" height="48" viewBox="-5.0 -10.0 110.0 135.0" fill={color}>
          ...
          {dressCodes.map((dc, idx) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className="w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 p-1"
                  style={{ background: `${theme.accent}18`, border: `1px solid ${theme.accent}25` }}
                >
                  <DressCodeIconSvg iconKey={(dressCodeIcons || {})[dc]} size={26} color={theme.accent} strokeWidth={2} />
                </div>
